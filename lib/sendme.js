      ...
      function send(json, extension, filename) {
        ...
        case 'xml':
          if (json instanceof Array) {
            var wrap = {};
            wrap[filename] = json;
            res.send(xml(wrap, {declarations: true}));
          } else res.send(xml(json, {declarations: true}));
          break;
        case 'xlsx':
...
/*
switch (req.api.format) {
    case "csv":
      res.set({"Content-type": "application/csv"});
      res.send(tabular.delimit(result, opts));
      break;
    case "txt":
      res.set({"Content-type": "text/plain"});
      opts.separator = '\t';
    ...
    case "xml":
      ...
      if (result instanceof Array) {
        var wrap = {};
        wrap[/\w+(?=\.\w+$|$)/.exec(req.path)] = result;
        res.send(js2xmlparser("body", wrap));
      } else {
        res.send(js2xmlparser("body", result));
      }
      break;
    case "xlsx":
      var xl = require('excel4node');
      var wb = new xl.WorkBook();
      var ws = wb.WorkSheet('pos');
      var rows = tabular.array(result,opts);
      for (var r=1; r<=rows.length; r++) {
        for (var c=1; c<=rows[r-1].length; c++) {
          var v = rows[r-1][c-1];
          if (typeof v === 'string') {
      ...
      wb.write(/\w+(?=\.\w+$|$)/.exec(req.path)+".xlsx", res);
      break;
    default:
      res.json(result);
    }
